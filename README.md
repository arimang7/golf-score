...
페라리 디자인 시스템에서 영감을 받은 세련된 디자인과 음성 인식 기술을 결합한 골프 스코어 관리 애플리케이션입니다. 필드 위에서 번거로운 텍스트 입력 대신, 목소리만으로 4명의 점수를 실시간으로 기록할 수 있습니다.
...
### 1. 환경 설정
프로젝트 루트에 `.env.local` 파일을 생성하고 다음 정보를 입력합니다:
```env
